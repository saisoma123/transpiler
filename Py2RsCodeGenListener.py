...
if __name__ is not None and "." in __name__:
    ...
    from Python3Parser import Python3Parser
class Py2RsCodeGenListener(ParseTreeListener):
    ...
    def enterFuncdef(self, ctx:Python3Parser.FuncdefContext):
        self.output.write("%sfn " % ('\t' * self.indentCount))

        self.output.write("%s" % (ctx.NAME().getText()))

        self.output.write("%s" % (ctx.parameters().getText()))
    ...
    def enterSimple_stmt(self, ctx:Python3Parser.Simple_stmtContext):
        x = ctx.small_stmt()[0].getText().strip()
        if("=" in ctx.small_stmt()[0].getText() and "{" in ctx.small_stmt()[0].getText()):
            z = ctx.small_stmt()[0].getText().strip()
            a = re.split("=", z)
            b = re.split("[+]=", z)[0]
            self.output.write("let mut " + a[0] + " = HashMap::from([" + b[b.index("{"):].replace("{", "(").replace("}", ")").replace(":", ",") + "\n")
           # self.output.write()
        if("=" in ctx.small_stmt()[0].getText() and "[" in ctx.small_stmt()[0].getText()):
            a = re.split("=", x)
            lis = a[0].split(",")
            ...
            for l in lis:
              if l in self.set:
                j += 1
                if j == len(lis):
                  self.output.write("%s " % ('\t' * self.indentCount * 2) + '(' + a[0] +  ')' + " = " + 
                                a[1])
              else:
                self.set.append(l)
                j += 1
                if j == len(lis):
                  self.output.write("%s let mut " % ('\t' * self.indentCount * 2) + '(' + a[0] +  ')' + " = " + 
                                a[1])         
            self.output.write("\n")
    ...
    def enterReturn_stmt(self, ctx:Python3Parser.Return_stmtContext):
        self.output.write("%s return " % ('\t' * self.indentCount)
                           + ctx.testlist().getText() + ";\n")
        self.indentCount += 1
    ...
    def enterIf_stmt(self, ctx:Python3Parser.If_stmtContext):
        self.output.write("%s if " % ('\t' * self.indentCount) + ctx.test()[0].getText() + " {\n")   
        x = ctx.suite()[0].getText().strip() 
        y = x.split("\r\n")
        for z in y:
            a = re.split("[+]=", z)
            if a[0] in self.set:
              self.output.write("%s " % ('\t' * self.indentCount * 2) + z + "\n")
            else:
              self.set.append(a[0])
              self.output.write("%s let mut " % ('\t' * self.indentCount * 2) + z + "\n")            
        self.output.write("%s }\n" % ('\t' * self.indentCount))
        if([x.getText() for x in ctx.test()][1] != None):
          self.output.write("%s else if " % ('\t' * self.indentCount) + ctx.test()[1].getText() + " {\n")
          b = ctx.suite()[1].getText().strip() 
          c = b.split("\r\n")
          for d in c:
              e = re.split("[+]=", d)
              if e[0] in self.set:
                self.output.write("%s " % ('\t' * self.indentCount * 2) + d + "\n")
              else:
                self.set.append(e[0])
                self.output.write("%s let mut " % ('\t' * self.indentCount * 2) + d + "\n")
          #self.output.write("%s " % ('\t' * self.indentCount * 2) + ctx.suite()[1].getText() + "\n")
          self.output.write("%s }\n" % ('\t' * self.indentCount))
        self.indentCount += 1
